...
function plotP(R::Vector{Int},
  ...
  plot(layer(x=repeat(sep[1:k], outer=2), y=[sep[1:k]; sep[2:end]],
       ...
       Coord.cartesian(yflip=true, fixed=true, xmin=0.5, xmax=n + 0.5,
                       ymin=0.5, ymax=n + 0.5),
       Scale.color_discrete_manual(Gadfly.@colorant_str("#FFFFFF"),
...
function plotC(site::Vector{Int},
  ...
  plot(layer(x=repeat(1:m, outer=3), ymin=[zeros(Float64, m); noise; weak],
       ...
       Scale.color_discrete_manual(Gadfly.@colorant_str("white"),
