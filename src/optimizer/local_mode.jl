...
function computelocalmode!(v::Vector{Int},
  ...
  for b in 1:m
    ...
    for l in 1:k
      ...
      # noise
      lγ[1] += lbeta(priorC.A[1, b] + n1s[g, b],
                     priorC.B[1, b] + v[g] - n1s[g, b]) - tmp1[1]

      # weak signal
      lγ[2] += lbeta(priorC.A[2, b] + n1s[g, b],
                     priorC.B[2, b] + v[g] - n1s[g, b]) - tmp1[2]

      # strong signal but not characteristic
      tmp2[1, l] = lbeta(priorC.A[3, b] + n1s[g, b],
                         priorC.B[3, b] + v[g] - n1s[g, b]) - tmp1[3]

      # strong signal and characteristic
      tmp2[2, l] = lbeta(priorC.A[4, b] + n1s[g, b],
                         priorC.B[4, b] + v[g] - n1s[g, b]) - tmp1[4]
...
function computelocalmode!(state::State,
  ...
  for b in 1:m
    ...
    for l in 1:k
      ...
      # noise
      lγ[1] += lbeta(priorC.A[1, b] + state.n1s[g, b],
                     priorC.B[1, b] + state.v[g] - state.n1s[g, b]) - tmp1[1]

      # weak signal
      lγ[2] += lbeta(priorC.A[2, b] + state.n1s[g, b],
                     priorC.B[2, b] + state.v[g] - state.n1s[g, b]) - tmp1[2]

      # strong signal but not characteristic
      tmp2[1, l] = lbeta(priorC.A[3, b] + state.n1s[g, b],
                         priorC.B[3, b] + state.v[g] - state.n1s[g, b]) -
                   tmp1[3]

      # strong signal and characteristic
      tmp2[2, l] = lbeta(priorC.A[4, b] + state.n1s[g, b],
                         priorC.B[4, b] + state.v[g] - state.n1s[g, b]) -
                   tmp1[4]
